...
@section('title', 'New Case - Advocate Pro')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-8 flex items-center space-x-4">
        ...
        <div>
            <h1 class="text-3xl font-black text-gray-900 tracking-tight">Create New Case</h1>
            <p class="text-gray-500 font-medium mt-1">Register a new legal proceeding in the system.</p>
        </div>
    </div>
    ...
    <form action="{{ route('cases.store') }}" method="POST">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="relative group">
                        <label class="block text-[10px] font-bold text-gray-400 mb-1.5 uppercase tracking-widest group-focus-within:text-indigo-600 transition-colors">Case ID (Manual Override)</label>
                        <input 
                            type="text" 
                            name="case_number" 
                            value="{{ old('case_number') }}"
                            class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm transition-all font-mono"
                            placeholder="Auto-generated if blank (e.g. ०८१-WO-०००१)"
                        >
                        <p class="mt-1 text-[9px] text-gray-400 italic">Leave blank for automatic Nepali-formatted ID.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="relative group">
                            ...
                            >
                                <option value="WO" {{ old('case_type_code') == 'WO' ? 'selected' : '' }}>WO</option>
                                <option value="CP" {{ old('case_type_code') == 'CP' ? 'selected' : '' }}>CP</option>
                                <option value="CR" {{ old('case_type_code') == 'CR' ? 'selected' : '' }}>CR</option>
                                <option value="RE" {{ old('case_type_code') == 'RE' ? 'selected' : '' }}>RE</option>
                            </select>
                        </div>
                    </div>
                    
                    <input type="hidden" name="sequential_number" value="">
                </div>
            </div>
                        ...
                        <div class="relative group">
                            ...
                            >
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" {{ old('client_id') == $client->id ? 'selected' : '' }}>{{ $client->contact_person }}</option>
                                @endforeach
                            </select>
                        </div>
            ...
            <div class="flex items-center justify-end space-x-4 pb-12">
                ...
                <button type="submit" class="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all">Create Case</button>
            </div>
        </div>
    </form>
</div>
